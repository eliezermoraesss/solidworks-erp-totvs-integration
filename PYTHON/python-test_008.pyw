...
try:
    ...
    # Adiciona validação para os códigos na coluna 1
    valid_codigos = df_excel.iloc[:, 1].apply(lambda x: bool(re.match(r'^(C|M|E)\-\d{3}\-\d{3}\-\d{3}$', str(x))))

    # Adiciona validação para a quantidade na coluna 3
    valid_quantidades = pd.to_numeric(df_excel.iloc[:, 2], errors='coerce').notnull()
    ...
    if not valid_quantidades.all():
        ctypes.windll.user32.MessageBoxW(0, "Quantidades inválidas encontradas. As quantidades devem ser números e não podem estar em branco.", "Erro", 0)

    # Continua com o restante do código apenas se os códigos e quantidades estiverem válidos
    if valid_codigos.all() and valid_quantidades.all():
        # Remove espaços em branco da coluna número 2
        df_excel.iloc[:, 1] = df_excel.iloc[:, 1].str.strip()

        # Encontra códigos que são iguais entre SQL e Excel
        codigos_em_comum = df_sql['G1_COMP'].loc[df_sql['G1_COMP'].isin(df_excel.iloc[:, 1])].tolist()
        codigos_adicionados_bom = df_excel.iloc[:, 1].loc[~df_excel.iloc[:, 1].isin(df_sql['G1_COMP'])].tolist()
        codigos_removidos_bom = df_sql['G1_COMP'].loc[~df_sql['G1_COMP'].isin(df_excel.iloc[:, 1])].tolist()
