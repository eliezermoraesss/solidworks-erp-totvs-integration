...
class CascadingTable(QMainWindow):
    ...
    def add_table_row(self, row, item):
        ...
        expand_button.clicked.connect(lambda _, r=row, btn=expand_button: self.toggle_row(r, btn))
        self.table.setCellWidget(row, 2, expand_button)

    def toggle_row(self, row, button):
        current_text = button.text()

        if current_text == "+":
            button.setText("-")
            # Exibe a próxima linha
            self.table.setRowHidden(row + 1, False)
        else:
            button.setText("+")
            # Oculta todas as linhas abaixo
            self.table.setRowHidden(row + 1, True)
