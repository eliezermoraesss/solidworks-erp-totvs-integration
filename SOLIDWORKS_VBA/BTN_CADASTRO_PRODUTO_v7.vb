...
End Sub
Sub VerificarObrigatoriedadeDeCampoNcm(codigo As String, statusEditarCodigo As String)
    firstLetter = Left(codigo, 1)
    If firstLetter = "C" And editarCodigo = statusEditarCodigo Then
        ncmObrigatorio = False
    ElseIf firstLetter <> "C" And editarCodigo = statusEditarCodigo Then
        ncmObrigatorio = True
    End If
End Sub
        ...
        If Not IsEmpty(vConfParam) Then
            For j = 0 To UBound(vConfParam)
                ...
                Select Case partes(1)
                    Case "codigo"
                        Dim firstLetterAuto As String
                        codigo = CStr(vConfValue(j))
                        Call VerificarObrigatoriedadeDeCampoNcm(codigo, "Unchecked")
                    Case "caixaTextoCodigo"
                        Dim firstLetterManual As String
                        caixaTextoCodigo = CStr(vConfValue(j))
                        Call VerificarObrigatoriedadeDeCampoNcm(codigo, "Checked")
                    Case "editarCodigo"
                    ...
                    Case "ncm"
                        ...
                        If ncmObrigatorio = True Then
                            ncm = ValidarNCM(ncm)
                        End If
                    Case "pesoLiquido"
                        pesoLiquido = CStr(vConfValue(j))
                        ValidarPeso (pesoLiquido)
                    Case Else
                        End Select
                    End If
            Next j
...
End Sub
Function ValidarPeso(peso As String)
    ...
    pesoLiquidoDouble = Val(peso)
    If pesoLiquidoDouble <= 0 Then
        MsgBox "O Peso Líquido deve ser maior que zero!", vbExclamation, "CADASTRO TOTVS - Validar Peso"
        End
    End If
End Function
...
Call TratamentoDosCamposDescricao(descricaoProduto, descricaoProduto2)
   ...
   On Error GoTo ErrorHandler
    ...
    If Len(Trim(caixaTextoCodigo)) = 0 And editarCodigo = "Checked" Or Len(Trim(descricaoProduto)) = 0 Or tipo = "" Or unidade = "" Or armazem(0) = "" Or grupo(0) = "" Or centroDeCusto(0) = "" Or (ncmObrigatorio = True And ncm = "") Then
        Err.Raise 9999
    End If
...
Sub ConsultarGrupoPeloCodigoRetornarDescricao(grupo As String)
    ...
    End If
End Sub
Function ValidarNcmNoTOTVS(codigoNcm As String) As String
    ' Defina as variáveis de conexão
    Dim cn As Object
    Set cn = CreateObject("ADODB.Connection")
    ...
    If cn.State = 1 Then ' 1 indica que a conexão está aberta
        ...
        Dim strSQL As String
        strSQL = "SELECT YD_TEC FROM " & baseDeDados & ".dbo.SYD010 WHERE YD_TEC = '" & codigoNcm & "' AND D_E_L_E_T_ <> '*';"
        ...
        Set cn = Nothing
    Else
        MsgBox "Falha na validação do NCM.", vbCritical, "CADASTRO TOTVS - Validar NCM"
    End If
End Function
...
Function ValidarNCM(ncm As String) As String
    ...
    ' Verifica se o NCM tem 8 caracteres
    If Len(ncmLimpo) <> 8 Then
        MsgBox "O código NCM deve ter 8 caracteres!", vbExclamation, "Validação NCM"
        End
    End If
    ...
    ncmExiste = ValidarNcmNoTOTVS(ncmLimpo)
    If ncmExiste = "" Then
        MsgBox "O código NCM não existe!" & vbNewLine & "Utilize um código existente e tente novamente.", vbExclamation, "Validação NCM"
        End
    End If
